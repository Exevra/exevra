...
Exevra v0.3.0 adds zero-argument Node initialization: it detects the package manager and common frameworks, safely configures Vitest's JUnit reporter, and gives explicit setup guidance for other runners. See the [supported frameworks reference](./reference/supported-frameworks/) for the compatibility contract. CI-neutral matrix aggregation remains available for explicit downloaded JUnit shard artifacts. Exevra does not assess test quality, assertions, code correctness, or whether a change is safe. It has no hosted service, accounts, GitHub API calls, pull-request comments, or remote artifact lookup.
