...
Exevra v0.2.0 adds CI-neutral matrix aggregation for explicit downloaded JUnit shard artifacts. Exevra does not assess test quality, assertions, code correctness, or whether a change is safe. It has no hosted service, accounts, GitHub API calls, pull-request comments, or remote artifact lookup. The npm package and Action tag are produced by the release workflow after the versioned change reaches `main`.
