...
For `aggregate`, missing configured shard directories produce `SHARD_MISSING`, missing reports in present shards produce `REPORT_MISSING`, and shards with no non-skipped tests produce `SHARD_NO_TESTS_EXECUTED`. Maven checks use `REPORT_MISSING` when a buildable module produces neither standard report family and `REPORT_UNREADABLE` when a matched report cannot be read. These are error findings. Malformed reports and unsafe paths are operational errors. Aggregate checks always include a notice that changed-file comparison is unavailable.
